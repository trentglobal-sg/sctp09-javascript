...
if (isWeekend) {
    //
    let cost = adultCount * ADULT_PRICE_ON_WEEKEND + childrenCount * CHILDREN_PRICE_ON_WEEKEND;
    // regardless of weekend or weekday, calculate and apply the discount
    if (cost > 100) {
        cost = DISOCUNT_PERCENTAGE * cost;
    }

    console.log("The cost is", cost);

} else {
    let cost = adultCount * ADULT_PRICE_ON_WEEKDAY + childrenCount * CHILDREN_PRICE_ON_WEEKDAY;
    // regardless of weekend or weekday, calculate and apply the discount
    if (cost > 100) {
        cost = DISOCUNT_PERCENTAGE * cost;
    }
